...
const convertCerts = () => {
  try {
    ...
    const deviceCert = Array.from(base64.base64ToBytes(Config.GREENLIGHT_CERT))
    const deviceKey = Array.from(base64.base64ToBytes(Config.GREENLIGHT_KEY))
...
export const initNodeAndSdk = async (eventHandler: any, restoreMnemonic: string | undefined = undefined) => {
  try {
    ...
    const developerKey: number[] = keys?.deviceKey || []
    const developerCert: number[] = keys?.deviceCert || []
    ...
    if (restoreMnemonic) {
      ...
      nodeConfig = {
        ...
        config: { partnerCredentials: greenlightCredentials },
      }
    ...
    if (nodeConfig && apiKey) {
      ...
      const eventSubscription = await connect(connectRequest, eventHandler)
...
export const invoicePaymentHandler = async (scannedData: any) => {
  try {
    ...
    while (retryCount < retries) {
      ...
      if (
        responseString.includes('transport error') ||
...
export const getBTCPrice = async () => {
  try {
    ...
    await fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=zar')
      ...
      .then((data) => {
        console.log(data.bitcoin.zar)
        btcZarPrice = data.bitcoin.zar
      })
