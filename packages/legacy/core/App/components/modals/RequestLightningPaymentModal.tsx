...
const RequestPaymentModal: React.FC<RequestPaymentModalProps> = ({
  ...
  const checkIfBTCPriceWasFetched = () => {
    return (
      (btcZarPrice === undefined && currencyType === Currency.ZAR) ||
      (btcZarPrice === -1 && currencyType === Currency.ZAR)
    )
  }
  ...
  return (
    ...
    >
      <View style={globalTheme.ChatTheme.paymentModals.modalView}>
        ...
        {!startingNode && (
          <View>
            <View>
              ...
              />
              {currencyType === Currency.BITCOIN ? (
                <Text style={globalTheme.TextTheme.label}>
                  {btcZarPrice && btcZarPrice !== -1
                    ? '    (R' + ((Number(currencyAmount) / 100000000) * btcZarPrice).toFixed(2) + ')\n'
                    : btcZarPrice === -1
                    ? 'Fetching price'
                    : 'Problem getting BTC price'}
                </Text>
              ) : (
                <Text style={globalTheme.TextTheme.label}>
                  {btcZarPrice && btcZarPrice !== -1
                    ? '    (' + ((Number(currencyAmount) / btcZarPrice) * 100000000).toFixed(2) + ' Satoshis)\n'
                    : btcZarPrice === -1
                    ? 'Fetching price'
                    : 'Problem getting BTC price'}
                </Text>
            ...
            <TouchableOpacity
              style={
                !checkIfBTCPriceWasFetched()
                  ? globalTheme.ChatTheme.paymentModals.mainButton
                  : globalTheme.ChatTheme.paymentModals.disabledMainButton
              }
              onPress={() => {
        ...
        {paymentStatusDesc ? (
          <Text style={{ color: 'white', minHeight: 50 }}>{paymentStatusDesc}</Text>
        ) : (
